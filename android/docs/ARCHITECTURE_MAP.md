...
- `src/components/Robot3D.tsx` renders the 3D assistant viewport
- `src/context/NeuralContext.tsx` distributes neural/media state
- `src/hooks/*` manage speech, media, motion, and orchestration
- `src/firebase.ts` provides auth/db bootstrapping and Firestore error shaping
...
2. client checks `auth.currentUser`
3. client fetches ID token
...
### 3D assistant path
1. `Robot3D.tsx` lazy/split chunk loads
