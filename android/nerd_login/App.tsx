...
import { loginWithGoogle, logout } from './supabase';
...
function DiagnosticItem({ label, status, details }: { label: string, status: 'online' | 'offline' | 'warning', details: string }) {
  return (
    <div className="bg-industrial-gray/40 border border-white/5 p-4 rounded-sm flex items-start gap-4 backdrop-blur-md relative overflow-hidden group">
      <div className="absolute inset-0 metal-texture opacity-5 pointer-events-none" />
      ...
      <div className="flex-1 relative z-10">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] font-display font-black text-white uppercase tracking-[0.2em]">{label}</span>
          <span className={`text-[9px] font-mono uppercase font-black ${status === 'online' ? 'text-neon-green' : status === 'offline' ? 'text-red-500' : 'text-orange-500'}`}>{status}</span>
        </div>
        <p className="text-[10px] text-white/40 leading-relaxed font-mono">{details}</p>
      </div>
    </div>
  );
}
...
function AppContent() {
  ...
  const handleStartSystems = async () => {
    // CRITICAL: Call startSystems IMMEDIATELY to capture user gesture for media permissions.
    // Any state update or await before this can cause the browser to lose the gesture context.
    const systemsPromise = startSystems().catch(err => console.warn("Systems started with errors:", err));
  ...
  const [supabaseStatus, setSupabaseStatus] = useState<'checking' | 'configured' | 'missing' | 'rls_error' | 'error'>('checking');
  ...
  useEffect(() => {
    fetch('/api/health')
      .then(res => res.json())
      .then(data => {
        if (data.supabaseStatus === 'rls_error') {
          setSupabaseStatus('rls_error');
        } else if (data.supabaseStatus.startsWith('error')) {
          setSupabaseStatus('error');
          setSupabaseDetail(data.supabaseStatus);
        } else {
          setSupabaseStatus(data.supabaseConfigured ? 'configured' : 'missing');
        }
      })
      .catch(() => setSupabaseStatus('missing'));
  }, []);
